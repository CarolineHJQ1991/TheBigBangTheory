...
class bubbleView: UIView {
    ...
    override func drawRect(rect: CGRect) {
        ...
        let circleContainer = CGRectInset(self.frame, 75,75)
        CGPathAddEllipseInRect(curvedPath, nil, circleContainer)
        pathAnimation.path = curvedPath
        self.layer.addAnimation(pathAnimation, forKey: "positionAnimation")
        
        let scale_x = CAKeyframeAnimation(keyPath: "transform.scale.x")
        scale_x.duration = 1
        ...
        let scale_y = CAKeyframeAnimation(keyPath: "transform.scale.y")
